...
function MainMenu()
    ...
    DPropSheet_MenuContents:AddSheet(_U('sheetInv'), DPanel_Inventory_Tab, "gui/silkicons/backpack", true, true, _U('sheetInvDesc'))
    DPropSheet_MenuContents:AddSheet(_U('sheetStats'), DPanel_Skill_Tab, "gui/silkicons/user_red", true, true, _U('sheetStatsDesc'))
    DPropSheet_MenuContents:AddSheet(_U('sheetCrafting'), DPanel_Craft_Tab, "gui/silkicons/wrench", true, true, _U('sheetCraftingDesc'))
    DPropSheet_MenuContents:AddSheet(_U('sheetHelp'), DPanel_Advert_Tab, "gui/silkicons/help", true, true, _U('sheetHelpDesc'))
...
function skillsMenu(frame)
    local function statMenu(frame)
        if DPanel_Stat_Menu and DPanel_Stat_Menu:IsValid() then else
            ...
            DPanel_Stat_Menu.Paint = function()
                ...
                draw.DrawText(_U('yourName')..LocalPlayer():Nick(), "TargetIDMedium", 10, 25, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT)
                if LocalPlayer().XP then
                    draw.DrawText(_U('xp')..LocalPlayer().XP, "TargetIDMedium", 10, 45, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT)
                    draw.DrawText(_U('moneySign')..LocalPlayer().Money, "TargetIDMedium", 10, 65, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT)
                    draw.DrawText(_U('credits')..LocalPlayer().Credits, "TargetIDMedium", 10, 85, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT)
                    draw.DrawText(_U('hunger')..LocalPlayer().Hunger, "TargetIDMedium", 10, 125, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT)
                    draw.DrawText(_U('thirst')..LocalPlayer().Thirst, "TargetIDMedium", 10, 145, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT)
                    draw.DrawText(_U('stamina')..LocalPlayer().Stamina, "TargetIDMedium", 10, 165, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT)
                end
            end
    ...
    local function perkMenu(frame)
        local function boolToText(b)
            if (b) then
                return _U('perkOwned'), Color(0, 255, 0, 255)
            else
                return _U('perkNotOwned'), Color(255, 255, 255, 255)
            end
        end
    ...
    local function skillMenu(frame)
        ...
        if DPanel_Skill_Menu and DPanel_Skill_Menu:IsValid() then else
            ...
            DPanel_Skill_Menu.Paint = function()
                ...
                if LocalPlayer().XP then draw.DrawText(_U('xp')..LocalPlayer().XP, "TargetIDMedium", w - 18, 5, Color(255, 255, 255, 255), TEXT_ALIGN_RIGHT) end
                if (highlightSkill) then
                    ...
                    draw.DrawText(_U('skillCost')..skillTbl.XP.._U('xp2'), "TargetIDMedium", 5, h * 0.6 + 20, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT)
            ...
            local DButton_Unlock = createButton(DPanel_Skill_Menu, w * 0.085, h * 0.9 - 30, 200, 30, _U('skillUnlock'))
    ...
    if DPanel_Skills and DPanel_Skills:IsValid() then else
        ...
        DPropSheet_StatContents:AddSheet(_U('sheetStats'), DPanel_Stats_Tab, "gui/silkicons/report", true, true, _U('sheetStatsDesc2'))
        DPropSheet_StatContents:AddSheet(_U('sheetPerks'), DPanel_Perks_Tab, "gui/silkicons/award_star_gold_1", true, true, _U('sheetPerksDesc'))
        DPropSheet_StatContents:AddSheet(_U('sheetSkills'), DPanel_Skills_Tab, "gui/silkicons/chart_line", true, true, _U('sheetSkillsDesc'))
...
function craftMenu(frame)
    if DPanel_Crafting and DPanel_Crafting:IsValid() then else
        ...
        DPropSheet_CraftContents:AddSheet(_U('sheetCraftBasic'), DPanel_Basic_Tab, "icon16/medal_bronze_1.png", true, true, _U('sheetCraftBasicDesc'))
        DPropSheet_CraftContents:AddSheet(_U('sheetCraftAdv'), DPanel_Intermediate_Tab, "icon16/medal_silver_1.png", true, true, _U('sheetCraftAdvDesc'))
        DPropSheet_CraftContents:AddSheet(_U('sheetCraftLegend'), DPanel_Luxury_Tab, "icon16/medal_gold_1.png", true, true, _U('sheetCraftLegendDesc'))
        ...
        local function updateCrafts()
            local function addCraftType(frame, crafttype)
                for _, craft in pairs(DayZCraft[crafttype]) do
                    ...
                    DLabel_ItemIng:SetText(_U('craftYouNeed'))
                    ...
                    local DButton_BuyItem = createButton(Craft_Item, 5, 105, 90, 30, _U('craft'))
...
function menuControlsInfo()
    ...
    dFrame_ControlPanel:SetTitle(_U('controlCommands'))
    ...
    if UseNewShop then
        dLabel_Benefits:SetText(_U('controlCommandsNewShop'))
    else
        dLabel_Benefits:SetText(_U('controlCommandsOldShop'))
    end
...
function menuRulesInfo()
    ...
    dFrame_ControlPanel:SetSize(500, 450)
    dFrame_ControlPanel:Center()
    dFrame_ControlPanel:SetTitle(_U('serverRulesTitle'))
    dFrame_ControlPanel:SetVisible(true)
    dFrame_ControlPanel:SetDraggable(true)
    dFrame_ControlPanel:MakePopup()
    dFrame_ControlPanel:ShowCloseButton(true)
    ...
    local dLabel_Benefits = vgui.Create("DLabel")
    dLabel_Benefits:SetColor(Color(255, 255, 255, 255))
    dLabel_Benefits:SetText(_U('serverRules'))
    dLabel_Benefits:SetPos(20, 30)
    dLabel_Benefits:SetFont("TargetIDSmall")
    dLabel_Benefits:SizeToContents()
    dLabel_Benefits:SetParent(dFrame_ControlPanel)
